class Module
{
  ...
  Module(XYZ icenter, boolean simulate) {
    ...
    legs[0] = new Leg(new XYZ(icenter.x, icenter.y + MODULE_WIDTH/2 + LEG_Y_OFFSET, icenter.z), 0, simulate);
    legs[1] = new Leg(new XYZ(icenter.x, icenter.y - (MODULE_WIDTH/2 + LEG_Y_OFFSET), icenter.z), PI, simulate); 
  }
}
